...
const Navigation = () => {
    ...
    return ( 
        ...
        <nav className={` ${isOpen ? "flex" : "hidden"} absolute top-full left-0 bg-red-600/75 backdrop-blur-[2px] w-screen py-4 flex-col gap-5 items-center md:bg-transparent md:static md:w-auto md:flex-row md:flex md:px-5 md:pt-8`}>
            ...
            <Link href="/movies" className="hover:text-neutral-400 md:hover:text-red-600 transition-all">Movies</Link>
            <Link href="/tvseries" className="hover:text-neutral-400 md:hover:text-red-600 transition-all">Tv Series</Link>
        </nav>
        </>
     );
}
