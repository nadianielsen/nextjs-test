...
const Navigation = () => {
    ...
    return ( 
        ...
        <div className="w-full h-20 flex items-center justify-between md:hidden">
            {/* <Link href="/" className={`${montserrat.className} ml-5 text-xl`}>Movie<span className="font-bold text-red-600">Base</span></Link> */}
        ...
        <nav className={` ${isOpen ? "flex" : "hidden"} absolute top-full left-0 bg-red-600/75 backdrop-blur-[2px] w-screen py-4 flex-col gap-5 items-center md:bg-transparent md:static md:w-auto md:flex-row md:flex md:px-5 md:pt-8`}>
            <Link href="/" className={`${montserrat.className} text-2xl`}>Movie<span className="font-bold text-red-600">Base</span></Link>
